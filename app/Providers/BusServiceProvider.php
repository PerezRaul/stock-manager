...
final class BusServiceProvider extends ServiceProvider
{
    public function register(): void
    {
        ...
        $this->mergeConfigFrom(__DIR__ . '/../../config/bus.php', 'stock-manager.bus');

        if (app()->environment() === 'testing') {
            $config->set('stock-manager.bus.scan_dirs', array_merge(
                (array) $config->get('stock-manager.bus.scan_dirs'),
                [base_path('tests/**/*')],
            ));
        }
        ...
        if (StrUtils::contains(base_path(), 'testbench-core')) {
            ...
            $config->set('stock-manager.bus.scan_dirs', $scanDirs);
        }

        /** @var string[] $scanDirs */
        $scanDirs = $config->get('stock-manager.bus.scan_dirs');
    ...
    private function registerEventBus(array $scanDirs): void
    {
        ...
        $connection    = strval(config('stock-manager.bus.event.connection'));
        $configuration = config('stock-manager.bus.event.connections.' . $connection);
